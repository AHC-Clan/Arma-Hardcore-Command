...
// SECTION 1: INITIALIZATION AND SAFETY CHECKS
    ...
    if (count (allPlayers select {alive _x && _x inArea _area}) > 0) exitWith {
        ["", "킬하우스에 아직 적이 남아 있습니다."] spawn "BIS_fnc_showSubtitle", _x
    };
    ...
    {["", "킬하우스 생성 중..."] remoteExec ["BIS_fnc_showSubtitle", _x]
    ...
    {["", "킬하우스를 생성하는 중..."] remoteExec ["BIS_fnc_showSubtitle", _x]
    } forEach (allPlayers select {_x distance _start < _height});
...
// SECTION 8: ROOM ORGANIZATION
    ...
    {["", "킬하우스 생성 완료!"] remoteExec ["BIS_fnc_showSubtitle", _x]
    } forEach (allPlayers select {_x distance _start < _height});
    ...
    if !(_start getVariable "reset") then {
        {["", "킬하우스 클리어!"] remoteExec ["BIS_fnc_showSubtitle", _x]
    ...
    // Show reset message
    {["", "킬하우스 초기화."] remoteExec ["BIS_fnc_showSubtitle", _x]
    } forEach (allPlayers select {_x distance _end < (_height*2)});
};
