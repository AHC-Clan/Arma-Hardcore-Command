...
tsp_fnc_killhouse = {
    ...
    {["", "킬하우스 배치 중..."] remoteExec ["BIS_fnc_showSubtitle", _x]} forEach (allPlayers select {_x distance _start < _height});
    ...
    {["", "킬하우스 배치 완료"] remoteExec ["BIS_fnc_showSubtitle", _x]} forEach (allPlayers select {_x distance _start < _height}); deleteVehicle _blocker;
    ...
    if !(_start getVariable "reset") then {{["", "클리어!"] remoteExec ["BIS_fnc_showSubtitle", _x]} forEach (allPlayers select {_x distance _end < (_height*2)})};
    ...
    {["", "킬하우스 리셋"] remoteExec ["BIS_fnc_showSubtitle", _x]} forEach (allPlayers select {_x distance _end < (_height*2)});
};
...
tsp_fnc_lights = {  //-- Toggle lights on/off
	...
	tsp_fnc_lights_wait = true;
	{
		...
		playSound3D [if (damage _x > 0.1) then {getMissionPath "sound\off.ogg"} else {getMissionPath "sound\on.ogg"}, _x, false, getPosASL _x, 5, random 2 max 0.5 min 2, _distance];		
...
tsp_fnc_cof = {  //-- Course of fire
	...
	[name _unit + " 님이 " + ([time - _startTime, "MM:SS"] call BIS_fnc_secondsToString) + " 기록으로 완주했습니다. (" + str (_unit getVariable "fired") + " 발, " + str (count (_targets select {!alive _x})) + "/" + str (count _targets) + " 적중)"] remoteExec ["systemChat", 0];
...
tsp_fnc_action_hold = {
	params ["_object", "_title", "_icon", "_code", ["_condition", "true"], ["_once", false], ["_duration", 0.1], ["_args", []], ["_priority", 10]];
...
tsp_fnc_action = {  //-- Changes for public release in here
	...
	if (_type == "Heal") then {[_object, "치료", "\a3\ui_f\data\IGUI\Cfg\holdactions\holdAction_revive_ca", {[player] call tsp_fnc_pause_heal}] call tsp_fnc_action_hold};
	if (_type == "Spectate") then {[_object, "관전", "\a3\ui_f\data\IGUI\Cfg\holdactions\holdAction_forceRespawn_ca.paa", {[true] call tsp_fnc_spectate}, "tsp_param_spectate != 'Disabled'"] call tsp_fnc_action_hold};
	if (_type == "Teleport") then {[_object, "텔레포트", "\a3\ui_f_oldman\data\IGUI\Cfg\holdactions\map_ca.paa", {titleCut ["", "BLACK OUT", 0.5]; sleep 0.5; [false] spawn tsp_fnc_spawn_map}] call tsp_fnc_action_hold};
	if (_type == "Role") then {[_object, "역할 선택", "\a3\ui_f\data\IGUI\Cfg\holdactions\holdAction_requestLeadership_ca.paa", {[] spawn tsp_fnc_role}, "tsp_param_role"] call tsp_fnc_action_hold};
	...
	if (_type == "Weather") then {[_object, "날씨", "hold_weather.paa", {[] spawn {sleep 0.2; findDisplay 46 createDisplay "RscDisplayAttributesModuleWeather"}}, "true"] call tsp_fnc_action_hold};
	if (_type == "Music") then {[_object, "음악", "hold_music.paa", {[] spawn {sleep 0.2; findDisplay 46 createDisplay "RscDisplayAttributesModuleMusic"}}, "true"] call tsp_fnc_action_hold};
	if (_type == "Sleep") then {
		[
			_object, "시간변경", "\a3\ui_f_oldman\data\IGUI\Cfg\holdactions\holdAction_sleep2_ca.paa", {[] spawn tsp_fnc_action_sleep}, 
	...
	if (_type == "Garage") then {[_object, "개러지", "\a3\data_f_destroyer\data\UI\IGUI\Cfg\holdactions\holdAction_loadVehicle_ca.paa", {_this#3 spawn tsp_fnc_action_garage}, _conditon, false, 1, _params] call tsp_fnc_action_hold};
};
