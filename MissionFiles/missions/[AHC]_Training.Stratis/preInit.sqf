tsp_fnc_killhouse = {
    ...
    {["", "킬하우스 배치중..."] remoteExec ["BIS_fnc_showSubtitle", _x]} forEach (allPlayers select {_x distance _start < _height});
    ...
    {["", "킬하우스 배치됨"] remoteExec ["BIS_fnc_showSubtitle", _x]} forEach (allPlayers select {_x distance _start < _height}); deleteVehicle _blocker;
    ...
    if !(_start getVariable "reset") then {{["", "킬하우스 클리어!"] remoteExec ["BIS_fnc_showSubtitle", _x]} forEach (allPlayers select {_x distance _end < (_height*2)})};
...
tsp_fnc_cof = {  //-- Course of fire
	...
	[name _unit + " 님이" + ([time - _startTime, "MM:SS"] call BIS_fnc_secondsToString) + "의 기록으로 코스를 완주했습니다. " + str (_unit getVariable "fired") + " 총알이 발사되었습니다. " + str (count (_targets select {!alive _x})) + "/" + str (count _targets) + " 타겟이 적중했습니다."] remoteExec ["systemChat", 0];
